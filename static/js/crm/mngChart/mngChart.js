$(function(){
	...
	console.log(window.location.hash.substring(7))
	$("#nav1").html(htmlData[window.location.hash.substring(7)].nav+"图表").attr("href",window.location.href);
	$("#nav2").html(htmlData[window.location.hash.substring(7)].nav+"报表").attr("href",'mngChart_'+ window.location.href.substring( window.location.href.indexOf("#chart_")+10,window.location.href.length) +'Tbl.html');
		
		
	
	$("#collapseSeven ul li").click(function(){
		hf = $(this).find("a").attr("href");
		name = hf.substring( hf.indexOf("#chart_")+7,hf.length);
		tb(htmlData[name]);
		$("#nav1").html(htmlData[name].nav+"图表").attr("href",hf);
	 	$("#nav2").html(htmlData[name].nav+"报表").attr("href",'mngChart_'+ hf.substring( hf.indexOf("#chart_")+10,hf.length) +'Tbl.html');
		})
