					...
					if(str.responseData.length>0){
						for(var i=0;i<str.responseData.length;i++){		
							string += '  <tr id='+ str.responseData[i].id +'>\
	                            ...
	                            <td class="text-right">'+noData(str.responseData[i].rpayAmount)+'</td>\
	                            ...
	                            <td>'+noData(str.responseData[i].ivcName)+'</td>\
	                             ...
	                             <td>\
	                             <a title="编辑" class="updContBox" href="contract_repay.html?custID='+str.responseData[i].custId+'&cntrID='+str.responseData[i].cntrId+'&repID='+str.responseData[i].id+'"><i class="icon-pencil"></i></a>\
	                            </td>\
	                        </tr> ';					 
...
//-------------------------------------------------------------------------- 查看合同信息 ---------------------------------------------------------------------
	...
	$(document).delegate('.seeContBox','click',function(){
		...
		$.ajax({
			...
			success:function(str){
				console.log(str);
				 $("#seeContform #rpayAmount").val(str.responseData.rpayAmount);      //合同名称
				 $("#seeContform #rpayAmount").attr("repid",str.responseData.id);
				 $("#seeContform #rpayAmount").attr("invoiceid",str.responseData.invoiceId);
				 $("#seeContform #rpayAmount").attr("cntrid",str.responseData.cntrId);
				 $("#seeContform #rpayAmount").attr("custid",str.responseData.custId);
				 $("#seeContform #rpayMethod").val(str.responseData.rpayMethod);  
			     $("#seeContform #rpayDate").val(setDate(str.responseData.rpayDate)); 
				 $("#seeContform #rpayMsg").val(str.responseData.rpayMsg); 
			}
		})
